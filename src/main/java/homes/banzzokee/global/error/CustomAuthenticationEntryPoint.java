...
public class CustomAuthenticationEntryPoint implements AuthenticationEntryPoint {
  ...
  public void commence(HttpServletRequest request, HttpServletResponse response,
    ...
    } else if (exception == ACCESS_TOKEN_REQUIRED) {
      log.error("[commence] 엑세스 토큰 필요");
      setErrorResponse(ACCESS_TOKEN_REQUIRED, request, response);

    } else if (exception == ACCESS_TOKEN_IS_BLACKLIST) {
      log.error("[commence] 블랙리스트 등록된 토큰");
      setErrorResponse(ACCESS_TOKEN_IS_BLACKLIST, request, response);
